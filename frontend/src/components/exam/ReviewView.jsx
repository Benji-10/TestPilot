...
export default function ReviewView() {
  const { activeAttempt, setActiveView, setActiveAttempt, setGeneratingExam, addExam, examSettings } = useStore()
  ...
  async function generateNextExam() {
    ...
    try {
      ...
      const settings = examSettings[exam?.session_id] || {}
      const nextExam = await apiClient.generateExam({
        sessionId: exam.session_id,
        fileIds: [],
        instructions: `Focus on these weak areas: ${weakTopics.join(', ')}`,
        settings: { ...settings, topicFocus: weakTopics.join(', ') },
      ...
      <div className="scroll-y" style={{ flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ fontSize: 24, fontWeight: 300, color: grade.color, fontFamily: 'Instrument Serif, serif' }}>
              {grade.label}
            </span>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 28, fontWeight: 300, color: 'var(--ink-1)', marginBottom: 4 }}>
              {totalScore} <span style={{ fontSize: 16, color: 'var(--ink-3)' }}>/ {maxScore}</span>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12 }}>
            ...
            <StatRow label="Time taken" value={attempt.time_taken_seconds ? `${Math.round(attempt.time_taken_seconds/60)}m` : '—'} />
          </div>
        </div>
        ...
        {/* Questions */}
        <div>
          <h2 style={{ fontSize: 12, fontWeight: 500, color: 'var(--ink-2)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Question Review
          </h2>
  ...
  return (
    <div style={{ flex: 1 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {topics.map(t => (
          <span key={t} className="tag" style={{ borderColor: `${color}30`, color }}>
            {t}
          </span>
        ))}
        {topics.length === 0 && (
