...
const LatexBlock = ({ children, style }) => (
  <div dangerouslySetInnerHTML={{ __html: renderLatex(String(children || '')) }}
...
export default function ReviewView() {
  ...
  async function generateNextExam() {
    ...
    try {
      ...
      const nextExam = await apiClient.generateExam({
        sessionId: exam.session_id, fileIds: [],
        instructions: weakTopics.length ? `Focus on: ${weakTopics.join(', ')}` : '',
        settings, fromAttemptId: attempt.id
      })
  ...
  if (!attempt || !exam) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--ink-3)' }}>No attempt to review</div>
  }
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 11, color: 'var(--ink-3)' }}>Detailed feedback on each question below</p>
        </div>
        <button className="btn btn-ghost" onClick={() => setActiveView('session')} style={{ color: 'var(--ink-3)' }}>← Back</button>
      </div>
      ...
      <div className="scroll-y" style={{ flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            border: `3px solid ${grade.color}`, background: `${grade.color}14`,
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'
          }}>
            <span style={{ fontSize: 28, fontWeight: 300, color: grade.color, fontFamily: 'Instrument Serif, serif', lineHeight: 1 }}>{grade.label}</span>
            <span style={{ fontSize: 11, color: 'var(--ink-3)', marginTop: 2 }}>{pct}%</span>
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 34, fontWeight: 300, color: 'var(--ink-1)', marginBottom: 6 }}>
              {totalScore} <span style={{ fontSize: 17, color: 'var(--ink-3)' }}>/ {maxScore} marks</span>
            </div>
            <div className="progress-bar" style={{ maxWidth: 280 }}>
              <div className="progress-bar-fill" style={{ width: `${pct}%`, background: grade.color }} />
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 5, fontSize: 12 }}>
            <Stat label="Correct" value={`${questions.filter(q => q.isCorrect).length}/${questions.length}`} />
            {feedback.method_marks_total != null && <Stat label="Method marks" value={feedback.method_marks_total} />}
          </div>
        </div>
        ...
        {feedback.overall_comment && (
          <div style={{
            ...
            fontSize: 13, lineHeight: 1.8, color: 'var(--ink-2)', fontStyle: 'italic'
          }}>
            "{feedback.overall_comment}"
          </div>
        )}
        ...
        {/* Questions */}
        <div>
          <div style={{ fontSize: 11, color: 'var(--ink-3)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
            Question Review
          </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          {feedback.weak_topics?.length > 0 && (
            <button className="btn btn-primary" onClick={generateNextExam} disabled={generatingNext} style={{ flex: 1, justifyContent: 'center' }}>
              {generatingNext ? 'Generating…' : '→ Practice weak topics'}
            </button>
          )}
          <button className="btn" onClick={() => setActiveView('analytics')} style={{ flex: 1, justifyContent: 'center' }}>
            View analytics
          </button>
        </div>
...
function TopicList({ title, topics, color }) {
  return (
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 11, color, fontWeight: 500, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{title}</div>
